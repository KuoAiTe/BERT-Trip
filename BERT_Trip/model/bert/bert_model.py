...
class BertTripConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.topk = 10
        self.max_sequence_length = file_max_sequence_length[self.dataset]
        self.feature_sep = '|'
        self.time_series_sep = ','
        self.os_path = os.getcwd()
        self.saved_model_dir = f'./saved_model/{model_type}'
        self.pretrained_model_dir = f'{self.saved_model_dir}/pretrained/{self.dataset}'
        ...
        self.num_time_token = 2
        self.use_data_agumentation = use_data_agumentation
        self.num_extra_tokens = 0
        if self.add_user_token:
            self.num_extra_tokens += self.num_user_token
        if self.add_time_token:
            self.num_extra_tokens += self.num_time_token
        self.model_type = model_type
        ##
        self.vocab_size = self.poi_vocab_size
...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if self.position_embedding_type == "relative_key" or self.position_embedding_type == "relative_key_query":
            ...
            print(query_layer.shape, time_embeddings.shape)
