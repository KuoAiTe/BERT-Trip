...
#'Toro',
'Melb',
#'weeplaces_poi_1000_length_3',
#'weeplaces_poi_1000_length_5',
#'weeplaces_poi_1000_length_7',
#'weeplaces_poi_1000_length_9',
#'weeplaces_poi_2000_length_3',
#'weeplaces_poi_2000_length_5',
#'weeplaces_poi_2000_length_7',
#'weeplaces_poi_2000_length_9',
]
...
def encoder(X,context,keep_prob=0.5):
    ...
    with tf.variable_scope("encoder"):
        ...
        #0-1, 1-2, 23-24
        #print 'time_t',time_t
        # poi embedding concat time embedding
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(train_iters):
            ...
            while step < len(encoder_train) // batch_size:
                ...
                print(f'epoch:{epoch}, step:{step} loss: {_AEloss:.5f} gen_loss: {real_loss:.5f}, critic: {_critic_loss:.5f} Fake: {fake_loss:.5f} gan: {gan_loss:.5f}')
        ...
        test_f1,test_pairs_f1,ftest_f1,ftest_pairs_f1, result = test(sess=sess,test_variables=test_variables) #
        print('test', epoch, test_f1, result)
        return (max_F1[max_epoch]), (max_pair_F1)[max_epoch],test_f1,test_pairs_f1,ftest_f1,ftest_pairs_f1, result
...
def test(sess,test_variables):#,Test_Time=None
    ...
    while step < len(encoder_test) // batch_size:
        ...
        for v in range(len(otpredicting_logits_[:1])):
            ...
            t = otpredicting_logits_[v][1:length - 1]
            t = np.random.choice(len(voc_poi[:-3]), len(otpredicting_logits_[v][1:length - 1]))
            recommend = np.concatenate([[actual[0]], t], axis=0)
            ...
            result = [{'expected': print_vocab(actual), 'predict': print_vocab(recommend)}]
        step += 1
    #print 'trajectory length',length
    return np.mean(F1),np.mean(pairs_F1),np.mean(fake_F1),np.mean((fake_pairs_F1)), result
...
def get_data(index,K):
    ...
    while steps < len(testU) // batch_size:
        print("step", steps, len(testU), batch_size)
...
if __name__ == "__main__":
    ...
    for i in range(K):
        ...
        train_F1,train_pairs_F1,test_F1,test_pairs_F1,ftest_F1,ftest_pairs_F1, result = train(data)
        with open('./result/result.csv','a') as f:
            f.write(f'data/{tra_name}|DeepTrip|LOOCV|{i}|{result}\n')
