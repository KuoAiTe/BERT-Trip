...
class Load_data():
    ...
    def load_dataset_kfold(self,train_index, test_index, BATCH_SIZE):

        query_list = []
        trajs_list = []
        query_data = pd.read_csv('./train_data/'+self.city+'-query.csv', header = None, sep= ' ', names = ['startPOI', 'startTime', 'endPOI', 'endTime'])
        trajs_data = open('./train_data/'+self.city+'-trajs.dat','r')
        for i in query_data.index:
            query_list.append(query_data.loc[i].tolist())
        for line in trajs_data.readlines():
        ...
        trajs_list = np.array(trajs_list)

        query_val = query_list[test_index]
        trajs_val = trajs_list[test_index]
        ...
        # print(dt_train)
        dt_val = tf.data.Dataset.from_tensor_slices((query_val, trajs_val)).shuffle(len(query_val))
        dt_val = dt_val.batch(BATCH_SIZE, drop_remainder=False)

        return dt_train, dt_val, int(len(query_train) / BATCH_SIZE) + 1, int(len(query_val)/BATCH_SIZE) + 1
